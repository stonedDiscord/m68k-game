                        ...
                        uint8_t glyphs[N_GLYPHS][GLYPH_H])
{
    ...
    while (n < N_GLYPHS && (ch = fgetc(f)) != EOF) {
        if (ch != '{') continue;
        /* collect up to 8 hex values from this brace group */
        uint8_t vals[GLYPH_H];
        int count = 0;
        while (count < GLYPH_H) {
            /* scan for next '0' that starts '0x' */
            int c2;
            ...
            if (digits > 0) vals[count++] = (uint8_t)(v & 0xFF);
        }
        if (count == GLYPH_H) {
            memcpy(glyphs[n], vals, GLYPH_H);
            n++;
        }
    }
    fclose(f);
                       ...
                       const uint8_t glyphs[N_GLYPHS][GLYPH_H])
{
    ...
    fprintf(f, "static const uint8_t font8x8[128][8] = {\n");

    for (int i = 0; i < N_GLYPHS; i++) {
        fprintf(f, "    { ");
        for (int r = 0; r < GLYPH_H; r++)
            fprintf(f, "0x%02X%s", glyphs[i][r], r < GLYPH_H-1 ? ", " : "");
        fprintf(f, " },  /* 0x%02X %s */\n", i, glyph_label(i));
    }
...
int main(int argc, char **argv)
{
    if (argc != 4 ||
        ...
        fprintf(stderr,
            "Usage:\n"
            "  %s export font.h   font.bmp      (C header -> BMP for editing)\n"
            "  %s import font.bmp font.h         (BMP -> C header)\n"
            "  %s cache  font.h   font_cache.h   (C header -> precomputed cache)\n",
    ...
    if (strcmp(argv[1], "export") == 0) {
        ...
        printf("exported %s -> %s (%dx%d 1bpp BMP)\n",
        ...
        printf("imported %s -> %s (%d glyphs)\n",
               argv[2], argv[3], N_GLYPHS);
    } else {
        if (parse_header(argv[2], glyphs) != 0) return 1;
        if (emit_cache(argv[3], glyphs) != 0) return 1;
        printf("cached %s -> %s (%d glyphs)\n",
               argv[2], argv[3], N_GLYPHS);
    }
    return 0;
}
